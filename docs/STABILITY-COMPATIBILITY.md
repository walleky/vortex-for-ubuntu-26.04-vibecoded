...
`proton-vortex repair-vortex` reruns `install.sh` with `FORCE_REINSTALL=1`. This repairs the Vortex app install and Windows uninstall registry metadata while leaving Vortex AppData, downloads, staging folders, profiles, collections, and mod lists alone.

`proton-vortex-skyrim-se fix-skse-launcher` writes SKSE batch launchers on the Proton desktop and in the Skyrim game folder. It does not edit Vortex's private Dashboard tool state; users still choose the generated batch file as the Vortex tool target.
...
- SKSE/preflight launches write logs under `~/.local/share/proton-vortex/logs`; use `proton-vortex-skyrim-se last-log` when a dock launch fails silently.
